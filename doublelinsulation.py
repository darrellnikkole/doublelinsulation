...
def home_page_footer(population_limit, rootpath):
    ...
    for data_plus_plu in data_plus_plus:
        ...
        replaced_data= home_page_template.replace('aloomaidstate', state).replace('aloomaidstaturl', f'{rootpath}{short_url}/')
    ...
    for data_plus_plu1 in range(higest):
        ...
        replaced_data1= home_page_template.replace('aloomaidstaturl', f'{rootpath}{short_url}/{link_city}/').replace('aloomaidstate', city_p)
...
def state_url_data(state_rep, state_abbr_short, radioselect, population_limit, Pages_created_options, Checkboxselected, rootpath):
    ...
    def all_state_url(state_rep, Checkboxselected):
        ...
        for city in hrml_crens_one:
            ...
            if state_rep.lower() == state_p.lower() or state_rep.lower()[:5] == state_p.lower():
                ...
                v+= home_page_template.replace('aloomaidstaturl', f'{rootpath}{state_abbr_short.lower()}/{link_city}/').replace('aloomaidstate', city_p)
                adeola += 1
                if adeola< 10:
                    while True:
    ...
    def all_county_url(state_rep):
        ...
        for county in hrml_crens_one:
            ...
            if state_rep.lower() == state_p.lower() or state_rep.lower()[:5] == state_p.lower():
                v+= home_page_template.replace('aloomaidstaturl', f'{rootpath}{state_abbr_short.lower()}/{sub_county}/').replace('aloomaidstate', county_p)
                adeola += 1
                if adeola< 10:
                    while True:
                        picker_rand= random.randint(0,len(hrml_crens_one)-1)
...
def city_url_data(data_usage, data_usage1, state_abbr_short, population_limit, Pages_created_options, Checkboxselected, rootpath):
    ...
    for data_plus_plu1 in range(higest):
        ...
        all_table_data += home_page_template.replace('aloomaidstaturl', f'{rootpath}{state_abbr_short}/{link_city}/').replace('aloomaidstate', city_p)
        id+=1
        if id==50:
            break
...
def county_url_data(data_usage, state_short_code_county, rootpath):
    ...
    for all_dat in all_data:
        ...
        zip= clean_text(zip)
        all_table_data+= home_page_template1.replace('aloomaidcityname', county_name).replace('aloomaidzipcode', zip).replace('aloomaidcityurl', f'{rootpath}{state_short_code_county}/{sub_ciy}')
        id1+=1
        if id1==50:
            break
...
def create_sitemape_page(sitemaps, rootpath):
    try:
        parent_dir5 = f"{rootpath.replace('/', '',1)}sitemap.xml"
...
def create_home_page_url(confirm_home_pages, domain_site_name, rootpath, phone_number, Long_formurl, shortformurl, site_name, randomize_image, all_refomated_body):
    try:
        os.makedirs(f'{rootpath.replace('/', '')}') 
    except:
        pass
    ...
    complate_header= template_data_3.replace('aloomaidtitle', metatitle_homepage).replace('aloomaiddescription', metadescription_homepage).replace('aloomaiddomainname', f'https://{domain_site_name}{rootpath}').replace('aloomaidsitename', site_name)
    ...
    home_page= random_homepage_webpage(confirm_home_pages).replace('<title></title>', complate_header).replace('<meta content="" name="keywords">', '').replace('<meta content="" name="description">', '').replace('aloomaiddomainname', f'https://{domain_site_name}{rootpath}').replace('</head>', '<link rel="stylesheet" href="static/aloomaspecial/alooma.css"><link rel="stylesheet" href="static/aloomaspecial/alooma2.css"></head>').replace('</body>', '<script src="https://kit.fontawesome.com/c4c9a6665b.js" crossorigin="anonymous"></script></body>', 1).replace('col-sm', 'col-s').replace('col-md', 'col-m').replace('col-lg', 'col-l').replace('#State', f'USA').replace('aloomaidbody', replaced_entities)
    fp = open(f'{rootpath.replace('/', '',1)}index.html', "w", encoding='utf-8-sig')
    fp.writelines(home_page)
    fp.close()

    return f'<url><loc>https://{domain_site_name}{rootpath}</loc></url>'
...
def create_state_url(state_name, confirm_state_pages, domain_site_name, state_abbr_short, all_table_dat1, all_table_dat2, rand_cty, rand_cunty, rootpath, phone_number, Long_formurl, shortformurl, site_name, randomize_image):
    ...
    if all_table_dat2=='':
        state_page= random_state_webpage(confirm_state_pages).replace('<meta content="" name="keywords">', '').replace('<meta content="" name="description">', '').replace('aloomaiddomainname', f'https://{domain_site_name}{rootpath}{state_abbr_short}/').replace(county_remove, '').replace('aloomaidstatecity', all_table_dat1).replace('</body>', '<script src="https://kit.fontawesome.com/c4c9a6665b.js" crossorigin="anonymous"></script></body>', 1).replace('col-sm', 'col-s').replace('col-md', 'col-m').replace('col-lg', 'col-l').replace('#State', f'{state_name}, {state_abbr_short.upper()}').replace('[listcountycity]', f'{rand_cty}, {rand_cunty}').replace('aloomaiddomainname', f'https://{domain_site_name}{rootpath}{state_abbr_short}/')
    else:
        state_page= random_state_webpage(confirm_state_pages).replace('<meta content="" name="keywords">', '').replace('<meta content="" name="description">', '').replace('aloomaiddomainname', f'https://{domain_site_name}{rootpath}{state_abbr_short}/').replace('aloomaidstatecounty', all_table_dat2).replace('aloomaidstatecity', all_table_dat1).replace('</body>', '<script src="https://kit.fontawesome.com/c4c9a6665b.js" crossorigin="anonymous"></script></body>', 1).replace('col-sm', 'col-s').replace('col-md', 'col-m').replace('col-lg', 'col-l').replace('#State', f'{state_name}, {state_abbr_short.upper()}').replace('[listcountycity]', f'{rand_cty}, {rand_cunty}').replace('aloomaiddomainname', f'https://{domain_site_name}{rootpath}{state_abbr_short}/')
    
    fp = open(f'{rootpath.replace('/', '',1)}{state_abbr_short}/index.html', "w", encoding='utf-8-sig')
    fp.writelines(state_page)
    fp.close()
    

    return state_name, f'<url><loc>https://{domain_site_name}{rootpath}{state_abbr_short}/</loc></url>'
...
def create_city_page_url(state_name, url_state_structure, state_abbr_short, city_name, url_city_structure, confirm_city_pages, domain_site_name, county_web_contents1, all_table_data1, iframe, zip, rootpath, phone_number, Long_formurl, shortformurl, site_name, randomize_image):
    state_abbr_short= state_abbr_short.lower()
    try:
        os.makedirs(f'{rootpath.replace('/', '')}{state_abbr_short}/{url_city_structure}') 
    except:
        pass
...
def create_county_page_url(state_name, url_state_structure, state_abbr_short, county_name, url_county_structure, confirm_county_pages, domain_site_name, county_web_contents2, all_table_data2, rootpath, phone_number, Long_formurl, shortformurl, site_name, randomize_image):
    state_abbr_short= state_abbr_short.lower()
    try:
        os.makedirs(f'{rootpath.replace('/', '')}/{state_abbr_short}/{url_county_structure}') 
    except:
        pass
    ...
    county_page= county_page.replace('</head>', metatiledescription_data).replace('{{aloomaidtitle}}', metatitle_countypage).replace('{{aloomaiddescription}}', metadescription_countypage).replace('<meta content="" name="keywords">', '').replace('<meta content="" name="description">', '').replace('aloomaidcountycontents', county_web_contents2).replace('aloomaidcountytable', all_table_data2).replace('#State',  f'{county_name}, {state_abbr_short.upper()}').replace('"../static/', '"../../static/').replace('aloomaiddomainname', f'https://{domain_site_name}{rootpath}{state_abbr_short}/{url_county_structure}/').replace('col-sm', 'col-s').replace('col-md', 'col-m').replace('col-lg', 'col-l').replace('{{aloomaidbody}}', replaced_entities)
    
    fp = open(f'{rootpath.replace('/', '',1)}{state_abbr_short}/{url_county_structure}/index.html', "w", encoding='utf-8-sig')
    fp.writelines(county_page)
    fp.close()
    
    return f'<url><loc>https://{domain_site_name}{rootpath}{state_abbr_short}/{url_county_structure}/</loc></url>'
